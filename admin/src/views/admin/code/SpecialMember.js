...
export default function SpecialMember() {
  ...
  useEffect(() => {
    dispatch(setLoading(true));
    getApi("/codes/get-member-code", user.token)
      .then((res) => {
  ...
  useEffect(() => {
    setUserList([]);
    allUser?.map((doc) => {
      setUserList((v) => [...v, { label: doc.name, value: doc.id }]);
    });
  }, [allUser]);
  ...
  return (
    <div className="gap-4 lg:grid lg:grid-cols-2 ">
              ...
              onDelete={() => {
                Swal.fire({
                  ...
                  if (result.isConfirmed) {
                    dispatch(setLoading(true));
                    deleteApi(
                      `/codes/delete-member-code?memberCodeId=${d.data.id}`,
                      user.token
                    )
                      .then((res) => {
                        dispatch(setLoading(false));
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Add Special Member</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Select
              isMulti
