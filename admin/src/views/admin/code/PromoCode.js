...
export default function PromoCode() {
  ...
  const addFlashAds = () => {
    if (!price || !code) {
      setSubmit(true)
      return alert.info("Please select price, percentage and code");
    }
    dispatch(setLoading(true));
    postApi(`/codes/create-promo-code`, {
      offer:price,
      code:code,
  ...
  useEffect(()=>{
    dispatch(setLoading(true));
    getApi("/codes/get-promo-code",user.token).then(res=>{
      setCodeList(res.data.data);
      //console.log(res.data.data);
      res.data.data?.length>0?setSelectCard(res.data.data[0]):setSelectCard(null)
      dispatch(setLoading(false));
    }).catch(e=>{
  ...
  return (
    <div className="lg:grid lg:grid-cols-2 gap-4 ">
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Add Promo Code</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
          <InputGroup>
            <Input
            ...
            >
              <Select
                ...
                onChange={(e) => setPercentage(e.target.value==="Percentage"?true:false)}
                placeholder="BDT"
              >
