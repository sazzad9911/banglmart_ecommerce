...
export default function Flash() {
  ...
  const start = async () => {
    ...
    try {
      ...
      const res = await postApi("/product/create/flash", form, user.token);
      ...
      {flashSell && !submit && flashSell.length === 0 ? (
        ...
        <div className="gap-4 lg:grid lg:grid-cols-2">
          <div>
            <div  className="grid h-[300px] w-full items-center justify-center border ">
              {picture ? (
                <img 
                  ...
                  src={!picture?.name ? picture : URL.createObjectURL(picture)}
                  alt={"df"}
                />
              ) : (
                <div>
            ...
            <div className="text-center">Recommended File Size 1300*400 px</div>
            ...
            <Button
              ...
              colorScheme={
                title && picture && startAt && endAt && user.user.role === 4
                  ? "cyan"
                  : "blackAlpha"
              }
            >
          ...
          {flashSell?.length > 0 && (
            <FloatButton onClick={() => setAddClick(flashSell[0])} />
          )}
        </div>
      )}
    </div>
