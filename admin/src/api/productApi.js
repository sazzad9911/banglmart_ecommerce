...
export const getAllProduct=async(userId,page,perPage,search)=>{
    const res=await axios.get(`${url}/product/getAll?userId=${userId}${page&&perPage?`&page=${page}&perPage=${perPage}`:""}${search?`&search=${search}`:""}`)
    return res.data
}
...
export const getVariant=async()=>{
    const res=await axios.get(`${url}/variant/getSpecifications`)
    return res.data
}
